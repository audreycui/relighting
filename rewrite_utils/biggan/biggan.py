...
class DBlock(nn.Module):
    ...
    def forward(self, x):
        if self.preactivation:
            h = F.relu(
                x
            )  # This line *must* be an out-of-place ReLU or it will negatively affect the shortcut connection.
        else:
            h = x
        h = self.conv1(h)
...
class Generator(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if self.G_param == 'SN':
            self.conv_func = functools.partial(
                layers.SNConv2d,
                kernel_size=3,
                padding=1,
                num_svs=num_G_SVs,
            ...
            self.linear_func = functools.partial(
                layers.SNLinear,
                num_svs=num_G_SVs,
                num_itrs=num_G_SV_itrs,
                eps=self.SN_eps,
        ...
        bn_linear = (
            functools.partial(self.linear_func, bias=False)
            if self.G_shared
            else self.embedding_func
        )
        self.bn_func = functools.partial(
            layers.ccbn,
        ...
        self.shared = (
            self.embedding_func(n_classes, self.shared_dim)
            if G_shared
            else nn.Identity()
        )
        # First linear layer
        self.linear = self.linear_func(
        ...
        for index in range(len(self.arch['out_channels'])):
            ...
            if self.arch['attention'][self.arch['resolution'][index]]:
                if self.verbose:
                    print(
                        'Adding attention layer in G at resolution {}'.format(
                            self.arch['resolution'][index]
                        )
                    )
                self.blocks[-1] += [
                    layers.Attention(self.arch['out_channels'][index], self.conv_func)
                ]
    ...
    def init_weights(self):
        ...
        if self.verbose:
            print(
                'Param count for G'
                's initialized parameters: {}'.format(self.param_count)
            )
    ...
    def forward(self, z, y, embed=False, **kwargs):
        if embed:
            if y.ndim > 1:
    ...
    def forward_feature_injection(
        ...
        feat_masks=None,
        feat_betas=None,
        feat_tmpl_name='blocks.{}.0',
    ):

        z, ys = self.get_zys(z, y, embed)
        ...
        for index, blocklist in enumerate(self.blocks):
            ...
            # If no mask provided, use uniform ones mask
            feat_masks = {} if feat_masks is None else feat_masks
            # If no betas provided, use 1.0 for all layers
            feat_betas = {} if feat_betas is None else feat_betas
            if feats is not None:
                feat_name = feat_tmpl_name.format(index)
                feat_inj = feats.get(feat_name)
                if feat_inj is not None:
                    # print(f'injecting new features with shape: {feat_inj.shape}')
                    feat_inj = feat_inj.to(h.device)

                    # If only one mask is provided, resize to match the current layer size.
                    if isinstance(feat_masks, torch.Tensor):
                        feat_mask = nn.functional.interpolate(
                            feat_masks[None][None], feat_inj.shape[-1]
                        )
                    else:
                        feat_mask = feat_masks.get(
                            feat_name, torch.ones_like(feat_inj, device=h.device)
                        )
                    # If only one beta is provided, use for all layers
                    if isinstance(feat_betas, (float, int)):
                        feat_beta = feat_betas
                    else:
                        feat_beta = feat_betas.get(feat_name, 1.0)

                    # Inject features at masked location only
                    # with strength feat_beta in [0.0, 1.0]
                    # with 1.0 completey replacing old feats.
                    h = (
                        feat_beta * (feat_inj * feat_mask)
                        + ((1.0 - feat_mask) * h)
                        + (1 - feat_beta) * feat_mask * h
                    )
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.D_param == 'SN':
            ...
            self.embedding_func = functools.partial(
                layers.SNEmbedding,
                num_svs=num_D_SVs,
                num_itrs=num_D_SV_itrs,
                eps=self.SN_eps,
    ...
    'places365': {
        128: {
            96: {
                ...
                'G_ema': os.path.join(
                    root_url, 'biggan128_G_ema_places365-78c78abe.pth'
                ),
                'state_dict': os.path.join(
                    root_url, 'biggan128_state_dict_places365-3d39f6bb.pth'
                ),
            }
        },
        256: {
            96: {
                ...
                'G_ema': os.path.join(
                    root_url, 'biggan256_G_ema_ch96_places365-ac277771.pth'
                ),
                'state_dict': os.path.join(
                    root_url, 'biggan256_state_dict_ch96_places365-b4f6daf6.pth'
                ),
            },
            128: {
                ...
                'G_ema': os.path.join(
                    root_url, 'biggan256_G_ema_ch128_places365-6fb66feb.pth'
                ),
                'state_dict': os.path.join(
                    root_url, 'biggan256_state_dict_ch128_places365-7594847d.pth'
                ),
            },
        },
    },
    'imagenet': {
        128: {
            96: {
                ...
                'G_ema': os.path.join(
                    root_url, 'biggan128_G_ema_imagenet-c9706dfb.pth'
                ),
                'state_dict': os.path.join(
                    root_url, 'biggan128_state_dict_imagenet-4aad5089.pth'
                ),
            }
        },
    },
...
def fix_class(G, y):
    ...
    setattr(G, 'forward', forward)
    return G


class BigGANDuo(torch.nn.Module):
    def __init__(self, size=256) -> None:
        super().__init__()
        self.size = size
        self.class_sizes = ()
        self.biggan_imagenet = BigGAN(resolution=size, pretrained='imagenet')
        self.biggan_places365 = BigGAN(resolution=size, pretrained='places365')

    @property
    def z_dim(self):
        return self.biggan_imagenet.z_dim + self.biggan_places365.z_dim

    @property
    def n_classes(self):
        return self.biggan_imagenet.n_classes + self.biggan_places365.n_classes

    def forward(self, x, y, mask=None, embed=False):
        xi, xp = torch.split_with_sizes(
            x, (self.biggan_imagenet.z_dim, self.biggan_places365.z_dim), dim=-1
        )
        yi, yp = torch.split_with_sizes(
            y, (self.biggan_imagenet.n_classes, self.biggan_places365.n_classes), dim=-1
        )
        oi = self.biggan_imagenet(xi, yi, embed=embed)
        op = self.biggan_places365(xp, yp, embed=embed)
        if mask is None:
            mask = torch.ones_like(oi)
        if isinstance(mask, (int, float)):
            mask = torch.ones_like(oi) * mask

        return oi * mask + (1 - mask) * op
