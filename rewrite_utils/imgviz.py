...
def matrix_normed_btw_0_and_255(M, mmax=None, mmin=None):
    is_tensor = True if torch.is_tensor(M) else False
    _min = M.min() if mmin is None else mmin
    _max = M.max() if mmax is None else mmax
    maxx = torch.max(torch.abs(_min), torch.abs(_max)) if is_tensor \
        else np.max(np.abs(_min), np.abs(_max))
    M = torch.clamp(M, _min, _max) if is_tensor else np.clip(M, _min, _max)
    M /= maxx
    M *= 128
    M += 128
    return M
...
class ImageVisualizer:
    ...
    def heatmap(self, activations, unit=None, mode='bilinear',
                amax=None, amin=None, image=None, image_alpha=0.2, signed=False):
        '''
        ...
        byt = (colormap(((a - amin) / (1e-10 + amax - amin)).numpy()) * 255).astype('uint8')
        if image is not None:
            byt = byt[:,:,:3]
            byt = (byt.astype('float') * (1.0 - image_alpha) +
                    self.numpy_image(image).astype('float') * image_alpha).astype('uint8')
        return PIL.Image.fromarray(byt)
    ...
    def pytorch_masked_image(
        ...
        if mask is None:
            mask = self.pytorch_mask(act_or_grad, unit, level=level,
                                     percent_level=percent_level, largest=largest).cpu()
        border = border_from_mask(mask, thickness)
        inside = (mask & (~border))
        outside = (~mask & (~border))
        inside, outside, border = [d.float()
                                   for d in [inside, outside, border]]
    ...
    def pytorch_mask(self, grad_or_act, unit, level=None, percent_level=None,
        ...
        if level is None:
            level = self.level_for(
                grad_or_act, unit, percent_level=percent_level)
        upsampler = self.upsampler_for(a)
        if largest:
